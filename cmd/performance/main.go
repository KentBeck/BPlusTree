...
func main() {
	...
	treeWithBloom := bplustree.NewGenericBPlusTree[uint64](
		branchingFactor,
	...
	for _, fpr := range falsePositiveRates {
		...
		// We can't directly set bloom filter params in the generic implementation
		// So we'll create a new tree and use the bloom filter with the calculated parameters
		...
		// Resize the bloom filter with the calculated parameters
		tree.ResizeBloomFilter(numKeys, fpr)
