...
// BranchNode is an internal node in the B+ tree
type BranchNode[K any] struct {
	keys     []K
	children []Node[K]
}

// NewBranchNode creates a new branch node
func NewBranchNode[K any]() *BranchNode[K] {
	return &BranchNode[K]{
		keys:     make([]K, 0),
		children: make([]Node[K], 0),
	}
}

// Type returns the type of the node
func (n *BranchNode[K]) Type() NodeType {
	return Branch
}

// Keys returns the keys in the node
func (n *BranchNode[K]) Keys() []K {
	return n.keys
}

// Children returns the children of the node
func (n *BranchNode[K]) Children() []Node[K] {
	return n.children
}

// KeyCount returns the number of keys in the node
func (n *BranchNode[K]) KeyCount() int {
	return len(n.keys)
}

// IsFull returns true if the node is full
func (n *BranchNode[K]) IsFull(branchingFactor int) bool {
	return len(n.keys) >= branchingFactor-1
}

// IsUnderflow returns true if the node has too few keys
func (n *BranchNode[K]) IsUnderflow(branchingFactor int) bool {
...
func (n *BranchNode[K]) InsertKeyWithChild(key K, child Node[K], less func(a, b K) bool) {
...
func (n *BranchNode[K]) findInsertPosition(key K, less func(a, b K) bool) int {
...
func (n *BranchNode[K]) InsertKey(key K, less func(a, b K) bool) bool {
...
func (n *BranchNode[K]) DeleteKey(key K, equal func(a, b K) bool) bool {
...
func (n *BranchNode[K]) FindKey(key K, equal func(a, b K) bool) int {
...
func (n *BranchNode[K]) Contains(key K, equal func(a, b K) bool) bool {
	return n.FindKey(key, equal) != -1
}

// FindChildIndex returns the index of the child that should contain the key
func (n *BranchNode[K]) FindChildIndex(key K, less func(a, b K) bool) int {
...
func (n *BranchNode[K]) SetChild(index int, child Node[K]) {
...
func (n *BranchNode[K]) RemoveChild(index int) {
	if index < len(n.children) {
...
func (n *BranchNode[K]) MergeWith(separatorKey K, other *BranchNode[K]) {
...
func (n *BranchNode[K]) BorrowFromRight(separatorKey K, rightSibling *BranchNode[K], parentIndex int, parent *BranchNode[K]) {
...
func (n *BranchNode[K]) BorrowFromLeft(separatorKey K, leftSibling *BranchNode[K], parentIndex int, parent *BranchNode[K]) {
	...
	n.children = append([]Node[K]{leftSibling.children[lastChildIndex]}, n.children...)
